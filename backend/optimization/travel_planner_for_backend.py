...
def select_hotel_candidate(
    ...
    for dest in dest_by_id.values():
        ...
        if candidate is None or candidate_info["travel_cost"] < candidate["travel_cost"]:
...
def plan_itenerary(city: str, budget: int, days: int, people: int, start_datetime_str: str) -> str:
    ...
    osaka_lookup = build_osaka_transportation_lookup(osaka_transportation_data, valid_ids)
    osaka_return_lookup = build_osaka_return_lookup(osaka_transportation_data, valid_ids)
    ...
    for day in range(days):
        ...
        while True:
            ...
            for dest in destinations:
                ...
                min_hotel_time = get_min_hotel_travel_time(dest["id"], trans_lookup, dest_by_id, visited)
                if current_time + total_time + min_hotel_time > day_total_time:
                    continue
                total_cost = (travel_cost + (dest.get("fare") or 0)) * people
                if total_cost > remaining_budget:
                    continue
                candidate_info = {
                    "destination_id": dest["id"],
            ...
            current_time = candidate["arrival_offset"] + candidate["visit_time"]
            remaining_budget -= candidate["total_cost"]
            current_dest = candidate["destination_id"]

        # 最終日の場合は大阪への帰りを追加
        if day == days - 1:
            return_candidate = select_return_trip(current_dest, current_time, osaka_return_lookup, remaining_budget, day_total_time)
            ...
            hotel_candidate = select_hotel_candidate(current_dest, current_time, osaka_lookup, trans_lookup, dest_by_id, visited, remaining_budget, sightseeing_end_time, day_total_time)
            if hotel_candidate is not None:
                ...
                remaining_budget -= (hotel_candidate["travel_cost"] + hotel_candidate["visit_cost"]) * people
                current_dest = hotel_candidate["destination_id"]
        itinerary.append(day_plan)
        # もし観光施設（ホテル以外）すべて訪問済みなら終了
        nonhotel_ids = {d["id"] for d in destinations if not d.get("ishotel")}
