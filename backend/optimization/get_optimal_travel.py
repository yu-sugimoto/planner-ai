...
# Load input JSONファイル群
def load_data():
    with open("data/combined_with_info.json", "r", encoding="utf-8") as f:
        tourist_data = json.load(f)
    with open("data/transportation_costs.json", "r", encoding="utf-8") as f:
        transportation_data = json.load(f)
    with open("data/Osaka_to_all_spots.json", "r", encoding="utf-8") as f:
...
def plan_itinerary(city: str, budget: int, days: int, departure_time: int):
    """
    ...
    for day in range(days):
        ...
        if day == days - 1:
            ...
            if return_candidate is None:
                ...
                remaining_budget -= return_candidate["travel_cost"]
                current_dest = 0
        else:
            # その日の締めとしてホテルにチェックイン
...
if __name__ == "__main__":
    ...
    if len(args) != 5:
        print(
            "Usage: python get_optimal_travel.py <departure time(00:00)> <city> <budget> <days>"
        )
        sys.exit(1)

    input_departure_time = args[1]
    # departure_time を分換算（例："10:00" → 600）
    departure_time = int(input_departure_time.split(":")[0]) * 60 + int(
        input_departure_time.split(":")[1]
    )
    city = args[2]
    budget = int(args[3])
    days = int(args[4])
    ...
    try:
        itinerary = plan_itinerary(city, budget, days, departure_time)
